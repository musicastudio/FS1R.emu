...
https://github.com/user-attachments/assets/a0273990-debc-4b4f-8114-d6a7eb4e1ed9
...
- `tools/extract_presets.py` pulls the 1408 preset voices, the 384 preset performances and the 90 preset formant sequences out of the EPROM image into `presets/`; `tools/extract_vop3.py` pulls the effect DSP's microcode into `docs/vop3/`; `tools/ghidra_ctrl_dests.py` decompiles the 48 controller destination handlers, which Ghidra never turns into functions because only a pointer table reaches them
