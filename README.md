...
Against rgwan's digital recording of the built-in demo, which the unit plays from its own EPROM so the same byte stream drives both: median envelope correlation 0.975, worst 0.902, mean band error 5.2 dB and tilt 2.1 dB. Against the 36 frozen capture files, twenty-seven agree with the unit inside half a decibel of level; the velocity law, the EG decay to a level, the modulator chain and the second and third links of it are inside a tenth.

Nothing in `cal.h` is a guess any more. The filter EG's rate law, its depth scale and LFO2's rate were the last three, and the third hardware session read them: the EG off the CPU's own stage counter over the debug monitor, the other two off one recording. The AM word turned out to be seven bits with its sensitivity in eighths, the feedback gain is measured, and the DX7 conversion, the voice bank switch and the plugin's note shift parameter were bugs rather than models.

What is left, worst first: the effects, whose blocks are modelled from the Data List and read 17 to 29 dB against real impulse responses, the unvoiced pedestal above 4 kHz, the `all1`/`all2` grain geometry and the formant's window family, and the demo drum's note 41, which sits between two measured points of the key code table. [docs/fidelity_plan.md](docs/fidelity_plan.md) ranks them with the numbers; all of them are modelling work with their data already recorded, and none is waiting on hardware. [STATUS.md](STATUS.md) carries the KNOWN / INFERRED / UNKNOWN lists and the open work. Tiers 0 to 3 and 5 are done, so the engine, the library and console split, the plugin in all three formats, the front panel GUI, the licence and CI are all in, and Tier 4 is fidelity and verification.
