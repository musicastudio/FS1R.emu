...
- `tools/extract_presets.py` pulls the 1408 preset voices out of the EPROM image into `presets/`; `tools/extract_vop3.py` pulls the effect DSP's microcode into `docs/vop3/`; `tools/ghidra_ctrl_dests.py` decompiles the 48 controller destination handlers, which Ghidra never turns into functions because only a pointer table reaches them
...
Not done: nothing has been measured against real hardware yet, so none of the modelled constants are calibrated. That is now the only thing between this and a verified engine, since everything the firmware can settle has been settled. See Tier 4 of `TODO.md`.
