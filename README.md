...
- `docs/` research notes, the register map and engine description, `formant.md` and `skirt.md` (the voiced operator's grain train, and what the spectral skirt does to it), `aeg.md`, the plugin guide, `midi_dispatch.md` (how a Note On, a Control Change and both aftertouches get from the SCI0 interrupt to the tone generator), `interface_from_firmware.md` (the panel's cursor stops and controller set, read out of the EPROM's own screen tables), the VOP3 reference, data list and manual text, the formant patent
...
Partly done: rgwan recorded the whole 46 minute capture set off his FS1R's digital output board on 2026-09-18, four register-level sweep sessions followed, and the first round of constants is measured rather than modelled. The grain window's skirt law came out of the last of those exactly, without a fit: `docs/skirt.md`. The output path's fixed gain, the hard clip on the channel accumulator and the filter loop's insertion loss are now numbers off the tap, and the EG rate law, the carrier level step, the frequency scale and the note table all came back confirmed as they stood. The unvoiced and formant level laws, the filter's cutoff reading and AM sensitivity are measured and still modelled wrongly. Detune is finished: it turned out to be the EPROM's own key scaled table rather than a constant, matched over six octaves to half a cent, which took a vibrato off demo song 2 that the unit does not have (`docs/detune.md`). `docs/capture_0918.md` is the working and Tier 4 of `TODO.md` tracks what is left.
