...
int selftest(Synth& S) {
    ...
    ,
      };
      uint8_t out[608]; convert_dx7(vced, out); int bad = 0;
      static const int skipCommon[] = {0x0E, 0x11, 0x12, 0x13, 0x19, 0x1E, 0x20, 0x23, 0x24, 0x25, 0x26, 0x2C};   // the demo's edits
      for (int i = 10; i < 112; i++) { bool skip = false; for (int k : skipCommon) if (k == i) skip = true; if (!skip && out[i] != demo[i]) bad++; }
      for (int o = 0; o < 8; o++) { const uint8_t *a = out + 112 + o * 62, *b = demo + 112 + o * 62;
          for (int i = 0; i < 62; i++) {
              bool edited = (i >= 12 && i <= 19) || i == 1 || i == 2 || i == 7 || (i >= 21 && i <= 27) || i == 33 || i == 34;   // EGs, pitch, detune, time scaling, level scaling, senses
              if (o >= 2 && edited) continue;
              if (a[i] != b[i]) bad++; } }
      ck("DX7 conversion matches the unit's own DX-Acrd 4 bulk on every unedited byte", bad == 0); }
