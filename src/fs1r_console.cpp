...
static void write_wav(const char* path, const std::vector<float>& l, const std::vector<float>& r) {
    ...
    for (size_t i = 0; i < frames; i++) {
        pcm[2 * i] = (int16_t)std::max(-32767.0f, std::min(32767.0f, l[i] * 32767.f));
        pcm[2 * i + 1] = (int16_t)std::max(-32767.0f, std::min(32767.0f, r[i] * 32767.f));
        peak = std::max(peak, (double)fabs(l[i])); rms += (double)l[i] * l[i];
    }
    FILE* f = fopen(path, "wb"); if (!f) { printf("cannot write %s\n", path); return; }
    uint32_t dataBytes = (uint32_t)pcm.size() * 2, fmtLen = 16, riffLen = 36 + dataBytes;
    uint16_t one = 1, chn = 2, bits = 16, align = 4; uint32_t rate = SR, bps = SR * 4;
    ...
    printf("wrote %s: %d frames, peak %.3f, rms %.3f\n", path, (int)frames, peak, sqrt(rms / std::max<size_t>(1, frames)));
}
