...
**Detune is a curve, not 2 cents per step.** Measured precisely off the audio rather than off the analyzer's FFT bins, the hardware runs ±25.7 cents at detune ±15, against the engine's ±30, and the shape is wrong as well as the scale. It is about 1.21 cents per step near zero and 2.7 at the ends, symmetric to 0.04 cents. The analyzer's own least-squares fit of 1.537 cents per step is the best straight line through a curve. Since detune is a CPU-side conversion, the real answer is probably a table in the ROM that the engine is modelling instead of reading, which makes this one for `FS1R_DISASM` rather than for the capture.

**It is not.** The 2026-09-19 register capture reads register 0x80 back off the unit for every one of those detune settings and it is exactly linear in the step, sign-magnitude, no table anywhere. The curve is the chip's, so `cal::DETUNE_CENTS` is the wrong shape rather than the wrong value and only a register sweep, experiment 2 in `FS1R.unlock/docs/unknowns.md`, can fix it.
