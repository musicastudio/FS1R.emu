...
**Detune is a curve, not 2 cents per step.** *(Fixed 2026-09-20: `cal::DETUNE_CENTS` is the measured curve now, read off the audio rather than off the analyzer's FFT bins, and the engine matches the recording at all eleven steps.)* The hardware runs ±25.7 cents at detune ±15 against the old ±30, about 1.21 cents per step near zero and 2.7 at the ends, and it is not quite symmetric: +6 and +15 sit 0.63 and 0.22 cents short of their negatives, in a tone that holds to a millihertz over two seconds and reads the same on the third harmonic. The analyzer's own least-squares fit of 1.537 cents per step is the best straight line through a curve and the engine no longer uses one.

This is not a pitch error that stays quiet. Operators detuned against each other beat at the difference between them, so 65% too much detune is heard as a warble at nearly twice the rate, and demo song 2 "Full Tines" went out with a vibrato the unit does not have: three of its six carriers sit at detune +4, +7 and -7, which beat at 1.2 and 2.4 Hz on the hardware and did so at 2.1 and 4.2 Hz here. The unit's own recording of the song confirms it, the 200 to 400 Hz band peaking at 1.21 and 2.42 Hz where the engine peaked at 3.94 and 4.24.

The curve is the chip's, not a ROM table the engine could read instead. The 2026-09-19 register capture reads register 0x80 back off the unit for every one of these settings and it is exactly linear in the step, sign-magnitude, no table anywhere, so what the CPU sends is the step itself and the shape is what the YMP706 makes of it.

The capture stepped by three and played note 60 only, so two things are still open and `11_detune` asks for both: the sixteen steps in between, and whether detune is a fixed frequency offset or a fixed ratio. Those are the same number at one note and an octave apart four octaves down, and only the second question decides whether a detuned patch beats at the same rate across the keyboard or warbles faster the higher it is played. `FS1R.unlock/captures/sweep.py detune` is the same measurement as a register sweep and is the quicker of the two.

**AM sensitivity is roughly twice as deep as the engine makes it.** From `ams-0` to `ams-7` the hardware loses 11.29 dB and the engine loses 4.89. The per-step shape differs too, so the engine's linear `ams / 7` scaling of the channel AM attenuation is not the law. Note this is a mean level under an LFO, so the sensitivity curve cannot be read straight off these numbers. The 2026-09-19 register capture puts this on the chip's side for good: the `fms << 3 | ams` byte at image +0x90 matches the engine on every segment, so the CPU sends the sensitivity the engine thinks it does and the depth is what the chip makes of it.
